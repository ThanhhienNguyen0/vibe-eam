...
function message(
  ...
  return {
    id: `${code}:${affectedEntityId ?? "model"}:${ruleId ?? "none"}`,
    severity,
    code,
    message: text,
    affectedEntityId,
...
function ruleForViewpoint(viewpoint: Viewpoint, state: SidebarState): ViewpointRule {
  ...
  return {
    ...
    allowedConnectionRuleIds: state.connectionRules
      .filter((rule) => ruleAppliesToViewpoint(rule, viewpoint.id))
      .map((rule) => rule.id),
...
export function getAllowedConnectionTypes(
  ...
  const allowedTypeIds = new Set(
    getAllowedConnectionRules(state, sourceComponentTypeId, targetComponentTypeId, viewpointId)
      .map((rule) => rule.connectionTypeId)
  );
  return state.connectionTypes.filter((type) => allowedTypeIds.has(type.id));
}

export function validateConnectionInstance(
  ...
  if (connectionType && sourceType && targetType) {
    ...
    if (!rule) {
      add(
        ...
        message(
          "error",
          "CONNECTION_RULE_MISSING",
          `${sourceType.name} may not ${relationVerb(connectionType.name)} ${targetType.name} according to the active metamodel.`,
          connection.id,
          undefined,
          "metamodel",
          "connection-rule"
      ...
      add(
        ...
        message(
          "error",
          "VIEWPOINT_RULE_DENIED",
          `${rule.description} is not allowed in ${viewpointName}.`,
          connection.id,
          rule.id,
          "viewpoint",
          "viewpoint-rule"
...
export function validateViewpointCompliance(
  diagram: Diagram,
  viewpoint: Viewpoint,
  state: SidebarState
): ValidationResult {
  ...
  for (const component of diagramComponents) {
    ...
    if (viewpointRule.allowedComponentTypeIds.length > 0 && !viewpointRule.allowedComponentTypeIds.includes(componentType.id)) {
      add(result, message("error", "VIEWPOINT_COMPONENT_DENIED", `${componentType.name} is not allowed in ${viewpoint.name}.`, component.id, viewpointRule.id, "viewpoint", "viewpoint-rule"));
    }
    if (viewpointRule.visibleComponentTypeIds && viewpointRule.visibleComponentTypeIds.length > 0 && !viewpointRule.visibleComponentTypeIds.includes(componentType.id)) {
      add(result, message("warning", "VIEWPOINT_COMPONENT_HIDDEN", `${componentType.name} is hidden in ${viewpoint.name}.`, component.id, viewpointRule.id, "viewpoint", "viewpoint-rule"));
    }
  ...
  for (const connection of diagramConnections) {
    const connectionType = state.connectionTypes.find((type) => type.id === connection.connectionTypeId);
    if (connectionType && viewpointRule.allowedConnectionTypeIds.length > 0 && !viewpointRule.allowedConnectionTypeIds.includes(connectionType.id)) {
      add(result, message("error", "VIEWPOINT_CONNECTION_DENIED", `${connectionType.name} is not allowed in ${viewpoint.name}.`, connection.id, viewpointRule.id, "viewpoint", "viewpoint-rule"));
    }

    const rule = matchingConnectionRule(connection, state);
    if (rule && viewpointRule.allowedConnectionRuleIds.length > 0 && !viewpointRule.allowedConnectionRuleIds.includes(rule.id)) {
      add(result, message("error", "VIEWPOINT_RULE_DENIED", `${rule.description} is not allowed in ${viewpoint.name}.`, connection.id, rule.id, "viewpoint", "viewpoint-rule"));
    }
  }
...
export function validateRequiredRules(
  ...
  const requiredRuleIds = new Set([
    ...
    ...state.connectionRules
      .filter((rule) => rule.required && connectionRuleAllowedByViewpoint(rule, state, viewpoint.id))
      .map((rule) => rule.id)
  ]);
    ...
    for (const component of diagramComponents.filter((item) => item.componentTypeId === rule.sourceComponentTypeId)) {
      ...
      if (count < rule.minOccurrences) {
        add(
          ...
          message(
            rule.severity,
            "VALIDATION_RULE_MIN_OCCURRENCES",
            `${component.name}: ${rule.message}`,
            component.id,
            rule.id,
            "required-rule",
            "validation-rule"
...
export function validateDiagram(
  ...
  const result = emptyResult();
  const viewpoint = diagram.viewpointId ? state.viewpoints.find((item) => item.id === diagram.viewpointId) : undefined;
  const activeMetamodel = state.metamodel;

  if (diagram.metamodelId && diagram.metamodelId !== activeMetamodel.id) {
    add(
      result,
      message(
        "error",
        "METAMODEL_MISMATCH",
        `Diagram violates Metamodel: expected '${activeMetamodel.name}' but references '${diagram.metamodelId}'.`,
        diagram.id,
        activeMetamodel.id,
        "metamodel"
      )
  ...
  if (diagram.viewpointId && !viewpoint) {
    add(result, message("error", "VIEWPOINT_MISSING", `Viewpoint '${diagram.viewpointId}' does not exist.`, diagram.id, undefined, "viewpoint"));
  }

  for (const id of diagram.componentIds.filter((componentId) => !state.components.some((component) => component.id === componentId))) {
    add(result, message("error", "DIAGRAM_COMPONENT_MISSING", `Diagram references missing component '${id}'.`, diagram.id, undefined, "diagram"));
  }

  const diagramConnections = diagram.connectionIds
    .map((id) => state.connections.find((connection) => connection.id === id))
    .filter((connection): connection is ConnectionInstance => Boolean(connection));
  for (const id of diagram.connectionIds.filter((connectionId) => !state.connections.some((connection) => connection.id === connectionId))) {
    add(result, message("error", "DIAGRAM_CONNECTION_MISSING", `Diagram references missing connection '${id}'.`, diagram.id, undefined, "diagram"));
  }
  ...
  for (const connection of diagramConnections) {
    ...
    if (!diagram.componentIds.includes(connection.sourceComponentId) || !diagram.componentIds.includes(connection.targetComponentId)) {
      add(result, message("error", "CONNECTION_ENDPOINTS_HIDDEN", "Connection endpoints must both be visible in the diagram.", connection.id, undefined, "diagram"));
    }
  }
