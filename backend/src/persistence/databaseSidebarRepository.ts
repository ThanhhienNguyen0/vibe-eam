...
export interface SidebarStateRepository {
  read(): Promise<SidebarState | null>;
  write(state: SidebarState): Promise<SidebarState>;
...
export class PrismaSidebarStateRepository implements SidebarStateRepository {
  ...
  async read(): Promise<SidebarState | null> {
    return this.prisma.$transaction(async (tx) => {
      const metamodel = await tx.metamodel.findFirst({ orderBy: [{ isActive: "desc" }, { updatedAt: "desc" }] });
      ...
      const [componentTypes, connectionTypes, connectionRules, viewpoints, viewpointRules, validationRules, diagrams, components, connections] = await Promise.all([
        ...
        tx.diagram.findMany({
          where: { metamodelId: metamodel.id },
          include: { componentInstances: true, connectionInstances: true },
          orderBy: { createdAt: "asc" }
        }),
        tx.componentInstance.findMany({ where: { componentType: { metamodelId: metamodel.id } }, orderBy: { id: "asc" } }),
        tx.connectionInstance.findMany({ where: { connectionType: { metamodelId: metamodel.id } }, orderBy: { id: "asc" } })
      ]);
  ...
  async write(state: SidebarState): Promise<SidebarState> {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.metamodel.updateMany({ data: { isActive: false } });
      ...
      for (const item of state.diagrams) {
        const data = {
          metamodelId: item.metamodelId ?? metamodelId,
          viewpointId: item.viewpointId,
          name: item.name,
          description: item.description
        };
      ...
      await tx.connectionInstance.deleteMany({ where: { connectionType: { metamodelId }, ...missing(state.connections.map((item) => item.id)) } });
      await tx.componentInstance.deleteMany({ where: { componentType: { metamodelId }, ...missing(state.components.map((item) => item.id)) } });
      await tx.diagram.deleteMany({ where: { metamodelId, ...missing(state.diagrams.map((item) => item.id)) } });
    ...
    return (await this.read()) ?? structuredClone(state);
  }
