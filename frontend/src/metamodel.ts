import type { EamElement, EamModel, EamRelation, ElementType, Layer, RelationType } from "./types";
...
export function analyzeImpact(model: EamModel, startId: string, mode: ImpactMode): AnalysisStep[] {
  const elements = new Map(model.elements.map((element: EamElement) => [element.id, element]));
